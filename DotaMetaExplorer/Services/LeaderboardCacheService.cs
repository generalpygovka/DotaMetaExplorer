...
public class LeaderboardCacheService : BackgroundService
{
    ...
    public async Task UpdateLeaderboardCacheAsync()
    {
        ...
        var proPlayers = await client.GetFromJsonAsync<List<ProPlayer>>(Constants.proPlayers + Constants.tokenApi);
        ...
        if (proPlayers != null)
        {
            foreach (var proPlayer in proPlayers)
            {
                ...
                var playerResponse = await client.GetAsync(Constants.players+ $"/{proPlayer.AccountId}" + Constants.tokenApi);
