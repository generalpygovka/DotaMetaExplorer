...
public class PlayerController : ControllerBase
{
    ...
    public async Task<IActionResult> GetProPlayers()
    {
        var request = new HttpRequestMessage
        {
            ...
            RequestUri = new Uri(Constants.proPlayers + Constants.tokenApi),
        };
    ...
    public async Task<IActionResult> GetPlayers(int id)
    {
        ...
        {
            Method = HttpMethod.Get,
            RequestUri = new Uri(Constants.players + $"/{id}" + Constants.tokenApi),
        };
        using (var response = await _httpclient.SendAsync(request))
        {
            response.EnsureSuccessStatusCode();
    ...
    public async Task<IActionResult> GetLeaderboardSlow()
    {
        ...
        using (var response = await _httpclient.SendAsync(request))
        {
            ...
            foreach (var proPlayer in proPlayers!)
            {
                ...
                var playerResponse = await _httpclient.GetAsync(Constants.players + $"/{proPlayer.AccountId}" + Constants.tokenApi);
